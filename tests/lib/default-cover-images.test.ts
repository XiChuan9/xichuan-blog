...
describe('default cover images', () => {
  ...
  it('falls back to a bundled default cover when no explicit cover is set', () => {
    const cover = resolvePostCoverImage(
      ...
      { baseUrl: 'https://xichuan-blog.vercel.app' },
    )

    expect(cover).toMatch(/^https:\/\/xichuan-blog\.vercel\.app\/default-covers\/xichuan-cover-[1-3]\.jpg$/)
  })
  ...
  it('prefers the explicit cover image and resolves relative paths', () => {
    expect(resolvePostCoverImage(
      ...
      { baseUrl: 'https://xichuan-blog.vercel.app' },
    )).toBe('https://xichuan-blog.vercel.app/api/images/example.png')
  ...
  it('resolves the site-wide fallback cover image', () => {
    expect(resolveDefaultSiteCoverImage('https://xichuan-blog.vercel.app'))
      .toBe('https://xichuan-blog.vercel.app/default-covers/xichuan-cover-1.jpg')
  })
})
