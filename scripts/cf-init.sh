...
DB_NAME="${CF_D1_NAME:-xichuan-blog-db}"
R2_NAME="${CF_R2_NAME:-xichuan-blog-images}"
KV_NAME="${CF_KV_NAME:-xichuan-blog-cache}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: npm run cf:init -- [--with-kv] [--site-url=https://your-domain.com] [--db-name=xichuan-blog-db] [--r2-name=xichuan-blog-images] [--kv-name=xichuan-blog-cache]" >&2
      exit 1
      ;;
  esac
done
